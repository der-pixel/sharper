...
app.post("/upload", async function (req, res) {
  ...
  if (files) {
    ...
    if (Array.isArray(files.fileLoad)) {
      ...
      for (const image of files.fileLoad) {
        // convert the images
        await convert(
          image.data,
  ...
  async function convert(image: Buffer<ArrayBufferLike>, toFile: string) {
  ...
  async function toJpeg(image: Buffer<ArrayBufferLike>, toFile: string) {
    await sharp(image)
  ...
  async function toPng(image: Buffer<ArrayBufferLike>, toFile: string) {
    await sharp(image)
  ...
  async function toWebp(image: Buffer<ArrayBufferLike>, toFile: string) {
    await sharp(image)
  ...
  async function toAvif(image: Buffer<ArrayBufferLike>, toFile: string) {
    await sharp(image)
  ...
  async function toTiff(image: Buffer<ArrayBufferLike>, toFile: string) {
    await sharp(image)
...
function arrayBufferToString(buffer: Buffer) {
  ...
  const bufferNumbers: number[] = Array.from(bufferArray);
