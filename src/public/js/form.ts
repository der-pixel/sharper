...
function numberRegex(number: HTMLInputElement, type: number) {
  // on event keydown
  number.addEventListener("keydown", (e: KeyboardEvent) => {
  ...
  const regex = type == 100 ? /^[1-9]{1}[0-9]{0,2}/ : /^[1-9]{1}[0-9]{0,4}/;
  ...
  const numberValidated = number.value.match(regex);
  if (numberValidated) {
...
function uploadedFile(id: string, textId: string) {
  ...
  const text = <HTMLInputElement>document.getElementById(textId);
...
async function makeRequest(form: HTMLFormElement) {
...
async function sendForm(form: FormData): Promise<ImageResponse> {
  // fetch the form
